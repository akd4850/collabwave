					        ...
					        <div style="display: flex;">
					            ...
					            <input type="hidden" name="postNo" value="${post.postNo}">
					              <c:if test="${not empty sessionScope.emp.empCode}">  
						            <button type="submit" id="btn-comment-register" class="btn btn-info btn-fill">댓글등록</button>
								  </c:if>
					        </div>
					    </td>
					</tr>
    ...
    success: (resData) => {      
      ...
      $('.btn-edit-comment').on('click', function() {
        ...
        commentItem.find('.comment-content').html(`
          ...
          <button type="button" class="btn btn-primary btn-save-comment" data-comment-no="${cmmtNo}">저장</button>
        `);
...
$('#btn-comment-register').on('click', (evt) => {
    ...
    $.ajax({
      // 요청
      type: 'POST',
      url: '${contextPath}/detailFree/registerComment.do',
